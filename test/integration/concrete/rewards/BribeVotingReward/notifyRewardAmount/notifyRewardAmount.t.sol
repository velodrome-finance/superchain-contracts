...
contract NotifyRewardAmountIntegrationConcreteTest is BribeVotingRewardTest {
    ...
    function test_WhenAmountIsNotZero() external whenTokenIsNotReward whenTokenIsWhitelisted {
        ...
        uint256 bribeBalance = IERC20(token).balanceOf(address(leafIVR));
        ...
        // It should transfer amount from sender to bribe contract
        assertEq(IERC20(token).balanceOf(address(this)), senderBalance - amount);
        assertEq(IERC20(token).balanceOf(address(leafIVR)), bribeBalance + amount);
