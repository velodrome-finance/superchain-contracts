...
library Commands {
    ...
    /// @dev Assumes message has a command encoded
    ...
    /// @dev Assumes message has an address encoded
    /// @param _message The message to be decoded
    ...
    /// @dev Assumes message has an amount encoded
    /// @param _message The message to be decoded
    ...
    /// @dev Assumes message has the gauge creation parameters encoded
    /// @param _message The message to be decoded
    ...
    /// @dev Assumes message has a nonce encoded
    /// @param _message The message to be decoded
    ...
    /// @notice Returns the gauge and payload in a message with nonce
    /// @dev Assumes message has a nonce, gauge and payload encoded
    /// @param _message The message to be decoded
    function gaugeAndPayload(bytes calldata _message) internal pure returns (address, bytes calldata) {
        return
            (address(bytes20(_message[ADDRESS_OFFSET:ADDRESS_OFFSET + 20])), _message[ADDRESS_OFFSET + 20:NONCE_OFFSET]);
    }

    /// @notice Returns the owner encoded in the message
    /// @dev Assumes message has a owner encoded
    /// @param _message The message to be decoded
