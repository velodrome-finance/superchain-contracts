...
show_help() {
    ...
    echo
    echo "Creates RestrictedXERC20 deployment files and optionally deploys them"
    echo
    echo "Arguments:"
...
if [ ! -f "$target_file" ]; then
    ...
    sed -e "s/DeployRestrictedXERC20Template/DeployRestrictedXERC20${capitalized_chain}/g" \
        -e "s/template.json/${chain}.json/g" \
        -e "s/owner: address(0)/owner: ${bridge_owner}/g" \
...
SCRIPT_PATH="${target_file}:DeployRestrictedXERC20${capitalized_chain}"
echo $SCRIPT_PATH
